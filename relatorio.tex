...
\begin{equation}\label{powerposteriors:mainequation}
    \int_0^1\expectation_{\theta | D, t}[\log\{p(D | \theta)\}] = 
        \log\{p(D | t = 1)\} - \log\{p (D | t = 0)\} = \log \{p (D)\} 
\end{equation}

A equação~\ref{powerposteriors:mainequation} nos permite estimar $p (D)$
sem precisar calcular explicitamente a integral~\ref{marginilize_likelihood}.
