...
there is no threshold like there is on ABC-SysBio, we expect that the 
closer the value of $\beta$ is to $1$, the closer should be the produced 
simulation to the experimental measurements; this is explained by the 
fact that the power posterior distributions $p_0({\bm \theta})$ and 
$p_1({\bm \theta})$ are, respectively, the prior and posterior 
...
model with those parameter and produce simulations to be compared with
...
chosen subset of parameters is enough. With such experiment, we are able 
to identify what dynamics were created on the candidate models according 
to the estimated posterior distribution of parameters. 
...
of $\beta$, which is shown on figure~\ref{fig:girolami_snm_model1only},
