...
apply some properties of enzymatic reactions together with some 
algebraic simplifications.

The first change we can apply to the system is based on the principle of 
mass conservation. If we assume that there is no or very little 
concentration of [ES] at the beginning of the experiment, and that the 
...
same time, then because of conservation of mass of E$_0$ (initial 
concentration of the enzyme) we have that
\begin{equation}
    \text{[E$_0$]} = \text{[E] + [ES]}
    \label{eq:E_conservation}
\end{equation}
and because of conservation of mass of S$_0$ (initial concentration of 
the substrate) we have that
\begin{equation}
    \text{[S$_0$]} = \text{[S] + [ES] + [P]}
    \label{eq:S_conservation}
\end{equation}
Applying enzyme mass conservation on~\ref{eq:dSdt} and substrate mass 
conservation on~\ref{eq:dESdt} produces the equations:
\begin{subequations}
    \begin{align}
    \frac{d[\text{S}]}{dt}  & = 
            -k_f(\text{[E$_0$]} - \text{[ES]})\text{[S]} + k_r\text{[ES]} 
            \label{eq:dSdt_2} \\
    \frac{d[\text{ES}]}{dt} & =  
            k_f(\text{[E$_0$]} - \text{[ES]})\text{[S]} - (k_r + k_{cat}) \text{[ES]} 
            \label{eq:dESdt_2}
    \end{align}
\end{subequations}

If we apply With these simplifications, it is now possible to calculate the ratio of
change of every chemical species just by considering 
